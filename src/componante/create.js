...
import { Text, View, StyleSheet, TextInput, TouchableOpacity, Alert, SafeAreaView, ScrollView } from 'react-native'
...
const Create = () => {
    ...
    if (newToy.trim() && newPrice.trim()) {
      if (editingId !== null) {
        ...
        setToys(toys.map(toy => 
          toy.id === editingId ? { ...toy, name: newToy, price: newPrice } : toy
        ))
        setEditingId(null)
      } else {
        // Add new toy
  ...
  return (
    <View style={styles.container}>
      <Text style={{ textAlign: "left", fontWeight: 'bold', fontSize: 20, paddingLeft: 5 }}>Create Items</Text>
      ...
      >
        <Text style={{textAlign: 'center'}}>{editingId !== null ? 'Update Item' : 'Add Item'}</Text>
      </TouchableOpacity>
      ...
      {toys.map((toy) => (
        ...
        <View key={toy.id} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 10, margin: 5, backgroundColor: 'lightyellow', borderRadius: 10 }}>
